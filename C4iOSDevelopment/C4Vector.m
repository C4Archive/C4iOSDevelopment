 ...
 */
static dispatch_queue_t vectorQueue;
...
-(id)initWithX:(CGFloat)x Y:(CGFloat)y Z:(CGFloat)z {
    ...
    if (self) {
        ...
        dispatch_once(&onceToken, ^{
            vectorQueue = dispatch_queue_create("vectorQueue", DISPATCH_QUEUE_CONCURRENT);
        });
...
-(void)add:(C4Vector *)aVec {
    [self update];
    dispatch_sync(vectorQueue, ^(void) { 
...
-(CGPoint)point2D {
	return CGPointMake((CGFloat)vec3[0], (CGFloat)vec3[1]);
}
